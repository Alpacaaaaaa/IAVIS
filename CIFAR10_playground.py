...
train_loader = torch.utils.data.DataLoader(train_dset, batch_size=64, shuffle=True, num_workers=0)
test_loader = torch.utils.data.DataLoader(test_dset, batch_size=64, shuffle=False, num_workers=0)
#######################################################
...
class DeepNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool1(self.act(self.conv4(self.act(self.conv3(x)))))
        x = self.pool2(self.act(self.conv6(self.act(self.conv5(x)))))
...
class BnDeepNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool1(self.act(self.bn4(self.conv4(self.act(self.bn3(self.conv3(x)))))))
        x = self.pool2(self.act(self.bn6(self.conv6(self.act(self.bn5(self.conv5(x)))))))
        
        x = x.view(-1, 512)
        x = self.fc3(self.act(self.fcbn2(self.fc2(self.act(self.fcbn1(self.fc1(x)))))))
        ##################################################################
        return x
